...
class Graph_QMixer(nn.Module):
    def __init__(self, args, scheme):
        ...
        self.universal_embd_size = (self.n_agents + args.n_enemies) * 4
        self.att_embed_dim = 10
        ...
        self.gat_module = GAT(scheme["obs"]["vshape"], 64, self.universal_embd_size, 0.1, 0.2, 4)
        # self.mapping_net = nn.Linear(self.state_dim, args.n)
    ...
    def forward(self, agent_qs, states, obs_ls, adj_ls):
        ...
        obs_ls = obs_ls.reshape(-1, self.n_agents, self.obs_size)
        adj_ls = adj_ls.reshape(-1, self.n_agents, self.n_agents)
        graph_feats = self.get_graph_info(states, obs_ls, adj_ls)

        agent_qs_ls = []
        for i in range(self.n_agents):
            # First layer
            w1 = th.abs(self.hyper_w_1(graph_feats[:,i,:]))
            b1 = self.hyper_b_1(graph_feats[:,i,:])
            w1 = w1.view(-1, self.n_agents, self.embed_dim)
            b1 = b1.view(-1, 1, self.embed_dim)
            hidden = F.elu(th.bmm(agent_qs, w1) + b1)
            # Second layer
            w_final = th.abs(self.hyper_w_final(graph_feats[:,i,:]))
            w_final = w_final.view(-1, self.embed_dim, 1)
            # State-dependent bias
            v = self.V(graph_feats[:,i,:]).view(-1, 1, 1)
