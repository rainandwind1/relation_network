...
class Multihead_QMixer(nn.Module):
    def __init__(self, args, scheme):
        ...
        self.embedding_encoder = nn.Linear(self.universal_embd_size, self.state_dim)
    ...
    def cal_KL_loss(self, ma_feats):
        kl_loss = 0
        for i in range(self.n_agents):
            for j in range(self.n_agents):
                kl_loss += F.kl_div(ma_feats[:,i,:].softmax(dim=-1).log(), ma_feats[:,j,:].softmax(dim=-1), reduction='none')
        return kl_loss

    def forward(self, agent_qs, states, obs_ls, train_mode = False):
        bs = agent_qs.size(0)
        ...
        if train_mode:
            kl_loss = self.cal_KL_loss(ma_feats)
            # reconstruction_loss = 0
            # for idx in range(self.n_agents):
            #     state_rec = self.embedding_encoder(ma_feats[:, idx, :])
            #     reconstruction_loss += (state_rec - states) ** 2
        ...
        if train_mode:
            ...
            kl_loss = kl_loss.sum(-1).view(bs, -1, 1)
            # reconstruction_loss = reconstruction_loss.sum(-1).view(bs, -1, 1)
            # print(kl_loss.shape, reconstruction_loss.shape)
            return kl_loss, 0., q_tot
        else:
            return q_tot
